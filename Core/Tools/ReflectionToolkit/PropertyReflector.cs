...
namespace Core.Tools.ReflectionToolkit
{
   /// <summary>
         ...
         if (!propertyInfoCache.ContainsKey(propertyName))
         {
            ...
            if (propertyInfo == null)
               throw new ArgumentException(
                  string.Format(CultureInfo.InvariantCulture, "Unable to find public property named {0} on type {1}",
               ...
               if (!_constructorCache.ContainsKey(type))
               {
                  ...
                  if (constructorInfo == null)
                     throw new Exception(string.Format(CultureInfo.InvariantCulture,
                        "Unable to construct instance, no parameterless constructor found in type {0}", type.FullName));
