...
namespace Core.Tools.WebToolkit
{
        ...
        private string GetContentSecurityPolicyValue()
        {
            ...
            if (string.IsNullOrWhiteSpace(contentSecurityPolicyErrorLogUri))
            {
                throw new NullReferenceException(
                   "Please set the `ContentSecurityPolicyErrorLogUri` value in `appsettings.json` file.");
            }
            ...
            "font-src 'self' data:",
            "img-src 'self' *.openstreetmap.org/ data: blob:",
            "connect-src 'self' https://elevation-api.io/",
            "media-src 'self'",
