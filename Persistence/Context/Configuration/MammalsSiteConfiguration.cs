...
{
   public class MammalsSiteConfiguration : IEntityTypeConfiguration<MammalsSite>
   {
      public void Configure(EntityTypeBuilder<MammalsSite> builder)
      {
         ...
         builder.HasMany(q => q.MammalsCensusSpecies).WithOne(w => w.Site).HasForeignKey(w => w.SiteId);
      }
   }
}
